...
class FetchUpdateZohoContacts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'auto:fetch_and_update_zoho_contacts';
     ...
     * @var string
     */
    protected $description = 'Fetch Zoho Contacts and update in database';
    ...
    public function handle()
    {
      ...
      // });
      $msg = "Cron For Automatic contacts update run";

      $msg = wordwrap($msg,70);

      mail("kuldeepsinghnetweb@gmail.com","Cron contacts update Test",$msg);
      $this->zoho_access_token   = ZohoApi::getAuthToken();
      $post_params = array (
        'per_page' => '200',
      );
      $this->ZohoUpdateContacts();
      if(is_array($this->contacts_data) && sizeof($this->contacts_data)){
        foreach ($this->contacts_data as $key => $contact) {
          ...
          if($find_contact){
              $find_contact->Owner_first_name       = $contact->{'Owner.first_name'};
              $find_contact->Owner_last_name        = $contact->{'Owner.last_name'};
              $find_contact->First_Name             = $contact->First_Name;
              $find_contact->Full_Name              = $contact->Full_Name; 
              $find_contact->Owner_id               = $contact->Owner->id; 
              $find_contact->Modified_Time          = $contact->Modified_Time;
              if($contact->Modified_Time){
                $find_contact->Modified_Time2       = substr($contact->Modified_Time, 0, 10);
              } 

              $find_contact->Last_Name              = $contact->Last_Name;
              if($contact->Created_Time){
                $find_contact->Created_Time         = $contact->Created_Time;
                $find_contact->Created_Time2        = substr($contact->Created_Time, 0, 10);
              }

              $find_contact->Docs_LP_Sale_Date      = $contact->Docs_LP_Sale_Date;
              
              if($contact->Docs_LP_Sale_Date){
                $find_contact->Docs_LP_Sale_Date     = substr($contact->Docs_LP_Sale_Date, 0, 10);
              } 
              $find_contact->save();
          }        
        }
    ...
    public function ZohoUpdateContacts($offset=NULL){
    $post              =  array();
    ...
    $post['select_query'] = "select Full_Name, First_Name, Last_Name, Modified_Time, Docs_LP_Sale_Date, Owner, Owner.first_name, Owner.last_name, Created_Time from Contacts where Modified_Time between '$last_2_hours' and '$end_time' $limit_query";
    ...
    if($jsonArrayResponse){
      ...
      if($jsonArrayResponse->info){
        if($jsonArrayResponse->info->more_records == 1) {
          $this->ZohoUpdateContacts($this->contacts_data_offset);
        }
      }
    }
    return NULL;
